...
public class ArrayUtils {
    ...
    public static List<String> stringToList(String data){
        return stringToList(data,REGEX_DEFAULT);
    }
     ...
     * @return
     */
    public static List<String> stringToList(String data,String regex){
    ...
    public static String listToString(List<String> datas){
        return listToString(datas,REGEX_DEFAULT);
    }
     ...
     * @return
     */
    public static String listToString(List<String> datas,String regex){
        String result="";
     ...
     * @return
     */
    public static <T> List<List<T>> getSubDatas(List<T> src, int subNum) {
        List<List<T>> result = new ArrayList();
        if (ObjectUtils.isEmpty(src)) {
            result.add(new ArrayList<>());
            return result;
        }
        int count = src.size() / subNum + (src.size() % subNum > 0 ? 1 : 0);
        for (int i = 0; i < count; i++) {
            List<T> data = new ArrayList<>();
            for (int j = i * subNum; j < i * subNum + subNum; j++) {
                if (j >= src.size())
                    break;
                data.add(src.get(j));
            }
