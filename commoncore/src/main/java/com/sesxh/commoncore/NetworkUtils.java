...
public final class NetworkUtils {
    ...
    public static Observable<Network> changeNet(NetworkType type,int timeout){
        ...
        return Observable.create(new ObservableOnSubscribe<Network>() {
            @Override
            public void subscribe(@NonNull ObservableEmitter<Network> emitter)  {
                int sportType=-1;
                NetworkRequest.Builder builder = new NetworkRequest.Builder();
                if (NetworkUtils.NetworkType.NETWORK_WIFI==type) {
                    sportType= NetworkCapabilities.TRANSPORT_WIFI;
                } else if (NetworkUtils.NetworkType.NETWORK_ETHERNET==type) {
                    sportType=NetworkCapabilities.TRANSPORT_ETHERNET;
                }else {
                    sportType=NetworkCapabilities.TRANSPORT_CELLULAR;
                }
                builder.addTransportType(sportType)
